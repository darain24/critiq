...
- **Default cache:** Cache entries are written atomically to `.cache/critiq.json` only when a caller supplies a cache key. Callers can replace the cache entirely.
